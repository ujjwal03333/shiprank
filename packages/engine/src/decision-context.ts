 ...
 * No LLM. Frequency is attached by the web layer when genome sample size is real.
 */
...
const CATALOG: Record<string, Omit<DecisionContext, "frequencyPct" | "sampleSize">> = {
  ...
  "INFRA-002": {
    aiPattern: "Errors only exist in the terminal that ran `next dev`",
    probableCause: "Sentry is an extra npm package the demo does not need to render.",
    whatShouldBe: "An error tracker initialized on the server and the client.",
    impactChain: "Production 500s are invisible until a user emails you.",
  },
  "INFRA-003": {
    aiPattern: "No product analytics",
    probableCause: "The prompt asked for a working UI, not instrumentation.",
    whatShouldBe: "PostHog, Plausible, or Vercel Analytics — gated behind consent if you use cookies.",
    impactChain: "You cannot tell a launch from a dead deploy.",
  },
  "INFRA-004": {
    aiPattern: "No /api/health",
    probableCause: "Uptime is not a screenshot the agent can see.",
    whatShouldBe: "A cheap GET that returns { status: 'ok' } for the monitor to hit.",
    impactChain: "The host is down and nobody knows until checkout fails.",
  },
  "INFRA-005": {
    aiPattern: "API_URL = 'http://localhost:3000'",
    probableCause: "The local demo needed a string, so the agent hardcoded one.",
    whatShouldBe: "process.env.NEXT_PUBLIC_APP_URL, with localhost only as a ?? fallback.",
    impactChain: "Production calls the developer's laptop.",
  },
  "INFRA-008": {
    aiPattern: "Dependencies freeze on whatever the agent installed",
    probableCause: "Dependabot is not part of the starter prompt.",
    whatShouldBe: "Weekly Dependabot or Renovate PRs.",
    impactChain: "Known CVEs sit in package-lock until someone remembers.",
  },
  "COMP-001": {
  ...
  "COMP-002": {
    aiPattern: "No terms of service",
    probableCause: "The checkout tutorial stops at 'Stripe session created'.",
    whatShouldBe: "A /terms route before you take money or host user content.",
    impactChain: "You have no contract when the first dispute arrives.",
  },
  "COMP-003": {
    aiPattern: "Analytics script dropped in with no consent",
    probableCause: "gtag is a one-liner; a consent banner is a product.",
    whatShouldBe: "Load non-essential cookies only after an explicit opt-in.",
    impactChain: "GDPR/ePrivacy exposure the moment an EU visitor lands.",
  },
  "COMP-004": {
    aiPattern: "Users can sign up but never leave",
    probableCause: "The prompt implemented auth, not GDPR erasure.",
    whatShouldBe: "DELETE /api/account that removes the user and related rows.",
    impactChain: "You cannot honour a deletion request without a manual SQL session.",
  },
  "COMP-005": {
    aiPattern: "console.log(user.email) for 'debugging'",
    probableCause: "The agent used the most convenient unique identifier.",
    whatShouldBe: "Log user IDs. Redact email, name, and phone.",
    impactChain: "PII in host logs is a reportable leak.",
  },
  "DATA-003": {
    aiPattern: "price FLOAT because it is a number",
    probableCause: "Tutorials use Float and never mention cents.",
    whatShouldBe: "NUMERIC(12,2) or integer cents.",
    impactChain: "Balances drift; refunds do not match charges.",
  },
  "DATA-004": {
  ...
  "DATA-006": {
    aiPattern: "for (const id of ids) await db.find(id)",
    probableCause: "The loop is the first thing the model writes.",
    whatShouldBe: "One query with WHERE id IN (...) or a JOIN.",
    impactChain: "List pages time out as soon as there are real rows.",
  },
  "DATA-007": {
    aiPattern: "Tables with no created_at",
    probableCause: "The dashboard wizard asked for columns, not audit fields.",
    whatShouldBe: "created_at / updated_at on every persistent table.",
    impactChain: "You cannot tell when a row appeared or last changed.",
  },
  "DATA-008": {
    aiPattern: "email TEXT with no UNIQUE",
    probableCause: "The agent stored the field and stopped.",
    whatShouldBe: "UNIQUE (email) or Prisma @unique.",
    impactChain: "Two accounts share an inbox; password reset hits the wrong one.",
  },
  "ARCH-001": {
    aiPattern: "createClient().from() inside 'use client'",
    probableCause: "The generated page is a client component so hooks work, then the query is pasted in.",
    whatShouldBe: "Fetch in a Server Component, Server Action, or service module.",
    impactChain: "The anon key and your schema leak to the browser bundle.",
  },
  "ARCH-002": {
    aiPattern: "One 800-line page that is the whole product",
    probableCause: "The agent keeps appending to the file that already works.",
    whatShouldBe: "Split by responsibility before the file crosses ~500 lines.",
    impactChain: "Every change risks the only working screen.",
  },
  "ARCH-004": {
    aiPattern: "Stripe and Supabase imported in the page",
    probableCause: "A service layer is extra files the demo does not render.",
    whatShouldBe: "One module per integration under services/ or lib/.",
    impactChain: "Swapping a vendor means grepping the whole app.",
  },
  "ARCH-006": {
    aiPattern: "One .env, used everywhere",
    probableCause: "The agent needed the app to boot, not an env matrix.",
    whatShouldBe: ".env.example committed, secrets in .env.local, never the prod DB locally.",
    impactChain: "A teammate's first run writes to production.",
  },
  "ARCH-008": {
    aiPattern: "The same button markup copied onto every page",
    probableCause: "A design system is not visible in the first screenshot.",
    whatShouldBe: "packages/ui or components/ui as the only source of shared chrome.",
    impactChain: "A colour change becomes a twenty-file hunt.",
  },
};
